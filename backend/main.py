        ...
        try:
            ...
            def run_agent():
                ...
                try:
                    ...
                    if task_info and task_info.get("uploaded_files"):
                        for f in task_info["uploaded_files"]:
                            # 正确处理 Windows 路径，找到第一个 ": uploaded file" 分隔符
                            if ": uploaded file" in f:
                                file_path = f.split(": uploaded file")[0].strip()
                                uploaded_file_paths.append(file_path)
                            elif f.startswith('/') or f.startswith('\\') or (len(f) > 1 and f[1] == ':'):
                                # 完整路径
